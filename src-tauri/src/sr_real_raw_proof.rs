...
const FIXTURE_ID: &str = "validation.computational-merge.super-resolution-subpixel.v1";
const REPORT_ID: &str = "computational-merge-run.super-resolution-subpixel.v1";
const SOURCE_DIR: &str = "private-fixtures/super-resolution/subpixel-detail-v1";
...
fn private_decode_smoke_generates_sr_real_raw_report_when_enabled() {
    ...
    run_private_sr_real_raw_decode_proof(&private_root)
        .expect("private super-resolution real RAW decode proof runs");
}

fn run_private_sr_real_raw_decode_proof(private_root: &Path) -> Result<(), String> {
    ...
    let decode_report_path = output_dir.join("sr-subpixel-decode-report.json");
    let quality_path = output_dir.join("sr-subpixel-quality.json");
    let report_path = output_dir.join("sr-subpixel-private-run-report.json");

    let metrics = build_metrics(&loaded_sources);
    if !metrics.iter().all(|metric| metric.passed) {
        return Err("Super-resolution private RAW decode metrics did not pass".to_string());
    }
    ...
    write_json(
        ...
        &SrDecodeReport {
            ...
            non_claims: vec![
                "not_registration_quality_verified".to_string(),
                "not_reconstruction_quality_verified".to_string(),
...
fn load_sources(private_root: &Path) -> Result<Vec<LoadedSrSource>, String> {
    ...
    SOURCE_RELATIVE_PATHS
        ...
        .map(|relative_path| {
            ...
            if !is_raw_file(&path_string) {
                return Err(format!("{relative_path}: expected NEF source"));
            }
...
fn validate_decoded_sources(sources: &[LoadedSrSource]) -> Result<(), String> {
    if sources.len() != SOURCE_RELATIVE_PATHS.len() {
        return Err(format!(
            "expected {} super-resolution sources, decoded {}",
...
fn build_metrics(sources: &[LoadedSrSource]) -> Vec<QualityMetric> {
