...
const FIXTURE_ID: &str = "validation.computational-merge.panorama-overlap.v1";
const REPORT_ID: &str = "computational-merge-run.panorama-overlap.v1";
const SOURCE_DIR: &str = "private-fixtures/panorama/overlap-stitch-v1";
...
fn private_decode_smoke_generates_panorama_real_raw_report_when_enabled() {
    ...
    run_private_panorama_real_raw_decode_proof(&private_root)
        .expect("private panorama real RAW decode proof runs");
}

fn run_private_panorama_real_raw_decode_proof(private_root: &Path) -> Result<(), String> {
    ...
    let decode_report_path = output_dir.join("panorama-overlap-decode-report.json");
    let quality_path = output_dir.join("panorama-overlap-quality.json");
    let report_path = output_dir.join("panorama-overlap-private-run-report.json");

    let metrics = build_metrics(&loaded_sources);
    if !metrics.iter().all(|metric| metric.passed) {
        return Err("Panorama private RAW decode metrics did not pass".to_string());
    }

    write_json(
        &decode_report_path,
        &PanoramaDecodeReport {
            ...
            non_claims: vec![
                "not_stitch_quality_verified".to_string(),
    ...
    let report = ComputationalMergePrivateRunReport {
        ...
        artifacts: vec![
            ...
            artifact(
                ...
                &format!("{ARTIFACT_ROOT}/panorama-overlap-decode-report.json"),
            )?,
            artifact(
                private_root,
                "quality_report_private",
                &format!("{ARTIFACT_ROOT}/panorama-overlap-quality.json"),
            )?,
        ],
        feature_family: "panorama_stitch".to_string(),
        fixture_id: FIXTURE_ID.to_string(),
        generated_at: Utc::now().to_rfc3339_opts(SecondsFormat::Millis, true),
        graph_revision_hash,
        implementation_issue: 1508,
        notes: "Private RAF panorama overlap direct decode smoke only. This proves production RAW loader ingest, nonzero decoded dimensions, finite decoded pixel payloads, source hashing, and metadata-only report collection. It does not claim panorama alignment, stitch quality, app-server apply, preview/export parity, or UI review.".to_string(),
...
fn load_sources(private_root: &Path) -> Result<Vec<LoadedPanoramaSource>, String> {
...
fn validate_decoded_sources(sources: &[LoadedPanoramaSource]) -> Result<(), String> {
    if sources.len() != SOURCE_RELATIVE_PATHS.len() {
        return Err(format!(
            "expected {} panorama sources, decoded {}",
...
fn build_metrics(sources: &[LoadedPanoramaSource]) -> Vec<QualityMetric> {
