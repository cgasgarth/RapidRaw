...
export const channelMixerRowSchema = z
  .object({
    blue: z.number().finite().min(-200).max(200),
    constant: z.number().finite().min(-100).max(100),
    green: z.number().finite().min(-200).max(200),
    red: z.number().finite().min(-200).max(200),
  })
  .strict();
...
export type ChannelMixerRow = z.infer<typeof channelMixerRowSchema>;
