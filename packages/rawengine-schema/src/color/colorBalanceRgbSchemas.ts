...
export const colorBalanceRgbRangeSettingsSchema = z
  .object({
    blue: z.number().finite().min(-100).max(100),
    green: z.number().finite().min(-100).max(100),
    red: z.number().finite().min(-100).max(100),
  })
  .strict();
