...
export const levelsSettingsSchema = z
  .object({
    enabled: z.boolean(),
    gamma: z.number().finite().min(0.1).max(5),
    inputBlack: z.number().finite().min(0).max(1),
    inputWhite: z.number().finite().min(0).max(1),
    outputBlack: z.number().finite().min(0).max(1),
    outputWhite: z.number().finite().min(0).max(1),
  })
  .strict()
