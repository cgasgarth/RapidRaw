...
bun perf run jobs.export-mixed-batch
bun perf run jobs.import-batch
bun perf run jobs.ai-capability-first-use-cold
bun perf run jobs.ai-capability-first-use-warm
bun perf run jobs.computational-hdr-merge
...
The jobs suite measures product terminals rather than launcher duration: a three-source mixed export must commit a completed three-output receipt; a six-source import must emit start/progress/committed-path/complete events and records first-visible latency, images/sec, MB/sec, and committed bytes; AI connector capability first use runs as separate cold and warm identities and requires one successful connector probe; and bounded HDR requires three ordered sources, an accepted native-style plan, and a correlated apply receipt. Each lane runs one warmup plus five retained samples through the shared persistent browser daemon.
...
`native.startup-shell-cold` uses a fresh home for every repetition; `native.startup-shell-warm` reuses one home after its warmup. The warm lane injects a validation-only persisted last-folder setting backed by a generated image, automatically restores that session through the real library path, and does not complete until the frontend reports a non-empty first viewport; `lastFolderFirstViewportMs` retains that terminal latency. Both require `RAWENGINE_PERF_STARTUP_BINARY`, launch a real app process, correlate the report PID and trace ID, validate monotonic process/window/frontend phases, retain first-paint/shell/interactive/frontend-ready IPC metrics, and require GPU plus library services to prove one editor-demand single-flight start. The dedicated native job runs them against the same bundle used for RAW validation.
