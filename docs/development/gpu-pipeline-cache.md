...
The registry fingerprints the exact core and optional WGSL sources, entry points, layout ABI versions, RapidRaw and WGPU versions, backend, adapter vendor/device, privacy-hashed adapter and driver details, requested feature words, and limits. Cache directories expose only the SHA-256 identity. Opaque bytes are loaded only when separate metadata has the same identity and the recorded artifact size and SHA-256 match. Invalid artifacts are quarantined and compilation continues without them.
...
On the maintained Apple Metal cold-cache validation run, WGPU correctly reported persistent-cache support as unavailable. Cold first render was 1,007.6 ms, the 30-run warm median was 8.3 ms (99.2% lower), output was pixel-identical, and diagnostics attributed 676 ms to core warmup/foreground wait. The same run proved the optional flare pipelines were absent for ordinary output and appeared only after the enabled flare request. Platform driver caches remain outside application control, so this proof always reports the current measured cold run instead of assuming a prior driver-cache state.
