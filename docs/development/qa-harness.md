...
bun qa run --watch --scenario browser.editor.compare
...
`--watch` preserves the initial scenario/tag selection, then debounces product, native, harness, integration, and build-configuration changes into affected-scenario reruns on the persistent daemon. Generated proof, build, dependency, and private-artifact paths are excluded so a receipt cannot trigger an infinite rerun loop. Changes arriving during a run collapse into one follow-up affected selection.

Receipts under `private-artifacts/qa/<run>/run.json` bind results to the Git SHA, dirty-worktree digest, worktree path, Bun lock/build identity, browser/platform, execution mode, seed, shard, durations, and exact rerun command. Failed browser scenarios retain a bounded server log, full-page PNG, and Playwright trace. Set `RAWENGINE_QA_VIDEO=1` when failure video is required; passing scenarios discard trace/video capture. `bun qa reproduce` validates the receipt, rejects another worktree's identity, and reruns failed scenarios (or the full recorded selection when all passed). Private artifacts are never committed.
...
`--persistent` starts or reconnects to a worktree-owned daemon through a mode-`0600` Unix socket and verifies its PID start token and worktree before reuse. Unchanged reruns retain Vite and Chromium. A source-identity change restarts the lightweight dev-server/browser session before the affected rerun because Vite's old HMR graph can leave a fresh Playwright navigation unresolved; lock/configuration or headed-mode changes use the same fail-closed restart. Every job still receives a fresh context; receipts distinguish unchanged reuse from source refreshes and count created, closed, and leaked contexts. Signals, explicit shutdown, and stale ownership recovery close browser contexts, Chromium, Vite, the socket, and state record.

`bun qa benchmark` executes the same scenario through one-shot, persistent-cold, and persistent-warm modes. It proves result equivalence, balanced context accounting, retained process identities, and a warm edit-to-result budget rather than using a synthetic command. Receipts accumulate server/browser starts and starts avoided, setup and scenario time, serialized worktree wait, artifact bytes, source reuse, restarts, failed-session recoveries, and context leaks.
