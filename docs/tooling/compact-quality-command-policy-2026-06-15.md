...
Quality gates should not flood local or CI logs by default. Package scripts that run noisy tools such as Biome, TypeScript, cargo checks, cargo audit, cargo deny, Bun audit, or actionlint must use `scripts/run-compact-command.ts`, ``, or delegate to another package script that does.
