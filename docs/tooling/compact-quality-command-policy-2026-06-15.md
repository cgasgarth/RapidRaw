...
Workflow `run` commands are parsed from YAML. Raw workflow commands are allowed only when listed in `scripts/check-compact-quality-commands.ts` with a bounded-output reason, such as Rust-only CI paths that intentionally avoid installing Bun.
