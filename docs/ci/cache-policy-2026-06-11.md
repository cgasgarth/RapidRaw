...
| Rust audit tool cache     | `.github/workflows/lint.yml`              | `~/.cargo/bin/cargo-audit`, `~/.cargo/bin/cargo-deny`        | OS, runner arch, tool name, pinned tool version    | Cache installed validation binaries, not Cargo build outputs.  |
