...
const REPORT_PATH = 'docs/validation/skin-tone-uniformity-local-raw-proof-2026-06-21.json';
...
const WORKFLOW_REPORT_PATH = `${request.artifactDirRelative}/skin-tone-uniformity-v1-workflow-report.json`;
...
const workflowReportSchema = z
  .object({
    ...
    editCommandId: z.literal('command.raw-open-edit-export.skin-tone-uniformity.v1'),
    editGraphRevision: z.literal('graph-rev.raw-open-edit-export.skin-tone-uniformity.v1'),
    fixtureId: z.literal('validation.raw-open-edit-export.skin-tone-uniformity.v1'),
    ...
    renderPaths: z
      ...
      .strict(),
    reportId: z.literal('raw-open-edit-export-run.skin-tone-uniformity.v1'),
...
const summaryReportSchema = z
  .object({
    doesNotProve: z
      .array(
        z.enum([
          'capture_one_skin_tool_equivalence',
          'face_detection_or_skin_segmentation',
          'full_macos_app_manual_session',
          'gpu_cpu_parity',
          'icc_colorimetric_accuracy',
          'portrait_or_film_stock_accuracy',
          'public_raw_fixture',
        ]),
      )
      .min(7),
    ...
    sourceRaw: z
      .object({
        fixtureStatus: z.literal('private_project_owned_raw_not_committed'),
        localPath: z.literal(request.sourceRelativePath),
        sha256: hashSchema,
      })
      .strict(),
    validationMode: z.literal('private_raw_skin_tone_uniformity_preview_export_sidecar_proof'),
...
if (UPDATE_REPORT) {
  ...
  summary = summaryReportSchema.parse({
    ...
    validationMode: 'private_raw_skin_tone_uniformity_preview_export_sidecar_proof',
...
if (requireAssets && privateRoot !== undefined) {
  ...
  if (workflowReport.reportId !== 'raw-open-edit-export-run.skin-tone-uniformity.v1') {
    failures.push('workflow report ID must match skin-tone uniformity proof run.');
  }
...
if (failures.length > 0) {
  console.error('skin-tone uniformity local RAW proof failed.');
  console.error(failures.join('\n'));
  process.exit(1);
}

console.log(`skin-tone uniformity local RAW proof ok (${requireAssets ? 'assets verified' : 'schema verified'})`);
