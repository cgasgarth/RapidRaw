...
const SOURCE_COMMAND = `RAWENGINE_PRIVATE_RAW_ROOT=${PRIVATE_ROOT} RAWENGINE_PRIVATE_RAW_SOURCE="${PRIVATE_SOURCE}" bun run check:denoise-real-raw-private-proof -- --require-assets`;
const ASSET_COMMAND = `RAWENGINE_PRIVATE_RAW_ROOT=${PRIVATE_ROOT} bun run check:denoise-real-raw-private-proof -- --require-assets`;
...
const privateWorkflowReportSchema = z
  .object({
    ...
    fixtureId: z.literal('validation.detail.denoise-real-raw.alaska.v1'),
    issue: z.literal(2890),
    ...
    proofClaims: z
      ...
      .strict(),
    reportId: z.literal('detail-denoise-real-raw.alaska.v1'),
    runtimeProof: z
      .object({
        decodePath: z.literal('load_base_image_from_bytes'),
        execution: z.literal('tauri_test_real_raw_denoise_stage'),
        macosAppUiE2e: z.literal(false),
        previewExportParityMetric: z.literal('previewExportMeanAbsDelta'),
        renderStage: z.literal('apply_denoise_stage'),
        sourceIsRaw: z.literal(true),
      })
      .strict(),
    validationMode: z.literal('private_raw_denoise_preview_export_parity'),
  })
  .passthrough()
  .superRefine((report, context) => {
    const metrics = metricMap(report.metrics);
    if ((metrics.get('previewExportMeanAbsDelta') ?? 1) > 0.000001) {
      context.addIssue({ code: 'custom', message: 'preview/export mean delta exceeded tolerance.' });
    }
    if ((metrics.get('inputToPreviewMeanAbsDelta') ?? 0) <= 0.00005) {
      context.addIssue({ code: 'custom', message: 'denoise did not alter the real RAW render enough.' });
    }
  });
...
const committedReportSchema = z
  .object({
    ...
    localRawRuntime: z
      .object({
        artifactRoot: z.literal('private-artifacts/validation/detail-denoise-real-raw'),
      ...
      .strict(),
    proofBoundary: z.literal('private_raw_runtime_not_final_denoise_quality'),
    proofStatus: z.literal('private_raw_denoise_preview_export_parity'),
    ...
    sourceRaw: z
      .object({
        ...
        localPath: z.literal('private-fixtures/detail/alaska-denoise-v1.arw'),
        sha256: hashSchema,
        sourceFolder: z.literal(PRIVATE_SOURCE),
      })
      .strict(),
    validationCommands: z.array(z.enum([SOURCE_COMMAND, ASSET_COMMAND])).length(2),
    validationMode: z.literal('local_alaska_raw_denoise_runtime'),
...
if (UPDATE_REPORT) {
  ...
  report = committedReportSchema.parse({
    ...
    issue: 2890,
    localRawRuntime: {
      artifactRoot: 'private-artifacts/validation/detail-denoise-real-raw',
    ...
    proofBoundary: 'private_raw_runtime_not_final_denoise_quality',
    proofStatus: 'private_raw_denoise_preview_export_parity',
    schemaVersion: 1,
    sourceRaw: {
      licenseEvidence:
        'User explicitly provided /Users/cgas/Pictures/Capture One/Alaska as project-owned RAW validation input for this repo.',
      licenseSummary: 'Project-owned local RAW sample for software development validation.',
      localPath: 'private-fixtures/detail/alaska-denoise-v1.arw',
    ...
    validationMode: 'local_alaska_raw_denoise_runtime',
...
if (requireAssets && privateRoot !== undefined) {
  const workflowReport = await readPrivateWorkflowReport();
  const workflowMetricValues = metricMap(workflowReport.metrics);
  if (
    requiredMetric(workflowMetricValues, 'previewExportMeanAbsDelta') !==
    report.localRawRuntime.metrics.previewExportMeanAbsDelta
  ) {
    failures.push('committed preview/export metric must match private workflow report.');
  }
...
if (failures.length > 0) {
  console.error('denoise real RAW private proof summary failed');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`denoise real RAW private proof summary ok (${requireAssets ? 'assets verified' : 'schema verified'})`);
