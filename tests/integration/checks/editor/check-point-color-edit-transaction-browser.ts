            ...
            .object({
              nodes: z
                .object({
                  point_color: z
                    .object({
                      params: z.object({ pointColor: z.object({ enabled: z.boolean() }).passthrough() }).strict(),
                    })
                    .passthrough(),
                })
                .passthrough(),
...
const waitForRenderedPointColor = async (page: Page, expected: boolean) => {
  const deadline = Date.now() + 10_000;
  while (Date.now() < deadline) {
  ...
  throw new Error(`Timed out waiting for rendered Point Color enabled=${String(expected)}.`);
};
...
try {
  ...
  if (
    ...
    throw new Error(`Point Color controls did not expose complete commit identity: ${JSON.stringify(identity)}`);
  }

  const enable = page.getByTestId('point-color-enable');
  ...
  if ((await enable.textContent())?.trim() !== 'Enabled') throw new Error('Point Color was not visibly enabled.');
  ...
  if (
    ...
    throw new Error(`Point Color did not persist one source-bound node revision: ${JSON.stringify(persisted)}`);
  }
  const rendered = await waitForRenderedPointColor(page, true);
  if (Object.hasOwn(rendered.args.request, 'jsAdjustments')) {
    throw new Error('Point Color render escaped node authority through jsAdjustments.');
  }

  const undo = page.locator('button[data-command-id="undo"]:visible').first();
  if (!(await undo.isEnabled())) throw new Error('Point Color commit did not create an undo boundary.');
  await undo.click();
  await page.waitForFunction(
    () => document.querySelector('[data-testid="point-color-enable"]')?.textContent?.trim() === 'Enable',
    undefined,
    { timeout: 10_000 },
  );
  ...
  console.error('point color edit transaction browser failed');
