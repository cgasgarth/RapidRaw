...
const resolveCargoArgs = () => {
  const args = ['test', '--manifest-path', 'src-tauri/Cargo.toml', 'wavelet_render', '--lib', '--', '--nocapture'];
...
const proc = Bun.spawn(['cargo', ...resolveCargoArgs()], {
  env: {
    ...process.env,
    RAWENGINE_WAVELET_WORKFLOW_PREVIEW_ARTIFACT: ARTIFACT_PATH,
    RAWENGINE_WAVELET_WORKFLOW_REPORT: REPORT_PATH,
  },
  stderr: 'pipe',
  stdout: 'pipe',
});
...
const report = parseWaveletWorkflowReport(JSON.parse(await readFile(REPORT_PATH, 'utf8')));
const artifact = await stat(report.artifactPath);
if (!artifact.isFile() || artifact.size < 128) {
  console.error(`Wavelet workflow artifact is missing or too small: ${report.artifactPath}`);
  process.exit(1);
}
if (report.inputToPreviewMaxDelta <= 0.0001) {
  console.error(`Wavelet workflow did not change pixels enough: ${report.inputToPreviewMaxDelta}`);
  process.exit(1);
}

console.log(
  `wavelet runtime ok delta=${report.inputToPreviewMaxDelta.toFixed(6)} artifact=${basename(report.artifactPath)}`,
);
