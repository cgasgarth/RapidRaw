...
const COMPACT_WRAPPERS = ['scripts/run-compact-command.mjs', 'scripts/run-compact-checks.mjs'];
...
const runsSelfContainedChecker = (command) => /^bun scripts\/check-[a-z0-9-]+\.mjs(?:\s|$)/u.test(command.trim());
...
const runSelfTest = () => {
  const packageViolations = checkPackageScripts({
    good: 'bun scripts/run-compact-command.mjs --label lint -- eslint .',
