...
const EXPECTED_FIXTURE_ID = 'validation.computational-merge.panorama-overlap.v1';
const EXPECTED_FEATURE_FAMILY = 'panorama_stitch';
const EXPECTED_ISSUE = 1508;
const EXPECTED_EXTENSION = '.raf';
...
if (args.selfTest) {
  await runSelfTest();
  console.log('panorama real RAW private root prep self-test ok');
  process.exit(0);
}
...
if (!result.ok) {
  console.error('panorama real RAW private root prep failed');
...
async function preparePrivateRoot(
  ...
  if (proofCase === undefined) return failure([`${EXPECTED_FIXTURE_ID}: missing panorama proof case.`]);
  ...
  if (proofCase.localSourceRelativePaths.length < 3) {
    failures.push(`${proofCase.fixtureId}: expected at least 3 panorama source paths.`);
  }
  ...
  const sourcePaths = proofCase.localSourceRelativePaths.map((sourcePath) => {
    if (extname(sourcePath).toLowerCase() !== EXPECTED_EXTENSION) {
      failures.push(`${sourcePath}: expected RAF source.`);
    }
  ...
  if (missingSources.length === sourcePaths.length && !requireAssets) {
    return {
      failures: [],
      message: `panorama real RAW private root prep skipped (add ${sourcePaths.length} RAF overlap frames under ${privateRoot})`,
      ok: true,
    };
  }
  ...
  return {
    failures: [],
    message: `panorama real RAW private root prep ok (${sourcePaths.length} sources)`,
    ok: true,
  };
}

async function runSelfTest(): Promise<void> {
  const root = await mkdtemp(resolve(tmpdir(), 'rawengine-panorama-private-root-'));
  try {
    for (const sourceName of ['frame-01.raf', 'frame-02.raf', 'frame-03.raf']) {
      const sourcePath = resolve(root, 'private-fixtures/panorama/overlap-stitch-v1', sourceName);
      await mkdir(dirname(sourcePath), { recursive: true });
      await writeFile(sourcePath, `fake-private-panorama-raw-${sourceName}`);
    }
    const manifest = parseComputationalMergeE2eProofManifest(
      JSON.parse(await readFile('fixtures/validation/computational-merge-e2e-proof.json', 'utf8')),
    );
    ...
    if (!result.ok || !result.message.includes('3 sources')) {
      throw new Error(result.failures.join('; ') || 'expected 3 panorama sources');
    }
  } finally {
