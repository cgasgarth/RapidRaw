...
const SOURCE_RELATIVE_PATH = 'private-fixtures/detail/alaska-denoise-v1.arw';
...
await runRequired('denoise committed proof summary', ['bun', 'run', 'check:denoise-real-raw-private-proof-summary']);
...
if (args.privateRoot === undefined) {
  ...
  console.log('denoise real RAW private proof skipped (RAWENGINE_PRIVATE_RAW_ROOT unset)');
  process.exit(0);
}
...
const resolution = await resolvePrivateRawRootSource({
  ...
  source: args.source,
  tempPrefix: 'rawengine-denoise-plain-root-',
});
...
await runRequired(
  ...
  [
    ...
    'denoise_real_raw_proof::private_runtime_smoke_generates_denoise_real_raw_report_when_enabled',
    '--',
    '--nocapture',
  ],
  {
    ...
    env: {
      ...
      RAWENGINE_RUN_PRIVATE_DENOISE_REAL_RAW_PROOF: '1',
    },
  },
);

await runRequired(
  'denoise private proof asset validation',
  [
    'bun',
    'run',
    'check:denoise-real-raw-private-proof-summary',
    '--require-assets',
...
console.log('denoise real RAW private proof ok');
...
async function preparePrivateRoot(privateRoot: string, source: string | undefined): Promise<void> {
  ...
  await mkdir(resolve(privateRoot, 'private-fixtures/detail'), { recursive: true });
...
async function resolveSourceRaw(source: string): Promise<string> {
  ...
  if (rawName === undefined) {
    throw new Error(`No RAW files found in ${sourcePath}.`);
  }
