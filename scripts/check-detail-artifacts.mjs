...
import { detailArtifactManifestSchema, parseDetailArtifactManifest } from '../src/schemas/detailValidationSchemas.ts';

const readJson = async (path) => JSON.parse(await readFile(path, 'utf8'));

const manifest = parseDetailArtifactManifest(await readJson('fixtures/detail/detail-artifacts.json'));
const invalidCases = await readJson('fixtures/detail/invalid-detail-artifacts.json');
const failures = [];
...
for (const artifact of manifest.artifacts) {
  if (artifact.capabilityState === 'cpu_reference_only' && artifact.limitations.includes('ui_api_wiring') === false) {
