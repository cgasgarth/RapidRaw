...
if (
  changedFiles.some((file) => file === 'package.json' || file.startsWith('.github/') || file.startsWith('scripts/'))
) {
...
for (const file of changedFiles) {
  if (!file.startsWith('scripts/check-') || !file.endsWith('.ts') || file.endsWith('check-current-pr-local.ts'))
    continue;
