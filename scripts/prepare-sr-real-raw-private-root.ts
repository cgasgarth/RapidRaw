...
const EXPECTED_FIXTURE_ID = 'validation.computational-merge.super-resolution-subpixel.v1';
const EXPECTED_FEATURE_FAMILY = 'super_resolution';
const EXPECTED_ISSUE = 1506;
const EXPECTED_EXTENSION = '.nef';
...
if (args.selfTest) {
  await runSelfTest();
  console.log('SR real RAW private root prep self-test ok');
  process.exit(0);
}
...
if (!result.ok) {
  console.error('SR real RAW private root prep failed');
...
async function preparePrivateRoot(
  ...
  if (proofCase === undefined) return failure([`${EXPECTED_FIXTURE_ID}: missing SR proof case.`]);
  ...
  if (proofCase.localSourceRelativePaths.length < 4) {
    failures.push(`${proofCase.fixtureId}: expected at least 4 burst source paths.`);
  }
  ...
  const sourcePaths = proofCase.localSourceRelativePaths.map((sourcePath) => {
    if (extname(sourcePath).toLowerCase() !== EXPECTED_EXTENSION) {
      failures.push(`${sourcePath}: expected NEF source.`);
    }
  ...
  if (missingSources.length === sourcePaths.length && !requireAssets) {
    return {
      failures: [],
      message: `SR real RAW private root prep skipped (add ${sourcePaths.length} NEF burst frames under ${privateRoot})`,
      ok: true,
    };
  }
  ...
  return {
    failures: [],
    message: `SR real RAW private root prep ok (${sourcePaths.length} sources)`,
    ok: true,
  };
}

async function runSelfTest(): Promise<void> {
  const root = await mkdtemp(resolve(tmpdir(), 'rawengine-sr-private-root-'));
  try {
    for (const sourceName of ['frame-01.nef', 'frame-02.nef', 'frame-03.nef', 'frame-04.nef']) {
      const sourcePath = resolve(root, 'private-fixtures/super-resolution/subpixel-detail-v1', sourceName);
      await mkdir(dirname(sourcePath), { recursive: true });
      await writeFile(sourcePath, `fake-private-sr-raw-${sourceName}`);
    }
    const manifest = parseComputationalMergeE2eProofManifest(
      JSON.parse(await readFile('fixtures/validation/computational-merge-e2e-proof.json', 'utf8')),
    );
    ...
    if (!result.ok || !result.message.includes('4 sources')) {
      throw new Error(result.failures.join('; ') || 'expected 4 SR sources');
    }
  } finally {
