...
const applyCommand = buildHdrMergeUiApplyCommandV1(
  {
    deghosting: 'high',
    outputName: 'Kitchen Window HDR',
    sources: [
...
if (applyCommand.parameters.acceptedDryRunPlanId !== 'hdr_ui_plan_001') {
  failures.push('HDR UI apply mapper must require and preserve accepted dry-run plan IDs.');
}
if (applyCommand.parameters.deghosting !== 'high') {
...
if (!blockedMissingPlan) {
  failures.push('HDR UI apply mapper accepted a missing acceptedDryRunPlanHash.');
}
