...
if (args.privateRoot === undefined) {
  ...
  console.log('SR real RAW private proof skipped (RAWENGINE_PRIVATE_RAW_ROOT unset)');
  process.exit(0);
}
...
await runCompact('SR real RAW private root prep', {
  command: ['bun', 'scripts/prepare-sr-real-raw-private-root.ts', ...(args.requireAssets ? ['--require-assets'] : [])],
  env: {
...
console.log('SR real RAW private proof ok');
