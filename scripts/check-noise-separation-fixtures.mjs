...
import { noiseSeparationFixtureSchema, parseNoiseSeparationFixtures } from '../src/schemas/noiseSeparationSchemas.ts';

const readJson = async (path) => JSON.parse(await readFile(path, 'utf8'));

const fixtures = parseNoiseSeparationFixtures(await readJson('fixtures/detail/noise-separation-fixtures.json'));
const invalidCases = await readJson('fixtures/detail/invalid-noise-separation-fixtures.json');
const failures = [];
