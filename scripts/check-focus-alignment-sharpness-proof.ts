  ...
  .object({
    alignment: z
      .object({
        ...
        command: z.literal('bun scripts/check-focus-translation-alignment-smoke.ts'),
    ...
    sharpness: z
      .object({
        ...
        command: z.literal('bun scripts/check-focus-sharpness-map-smoke.ts'),
        doesNotProve: z.array(z.string().trim().min(1)),
        fixtures: z.array(
          z
            .object({
...
const alignmentResult = run(['bun', 'scripts/check-focus-translation-alignment-smoke.ts']);
run(['bun', 'scripts/check-focus-sharpness-map-smoke.ts']);
...
const alignment = {
  ...
  command: 'bun scripts/check-focus-translation-alignment-smoke.ts' as const,
...
const sharpness = {
  ...
  command: 'bun scripts/check-focus-sharpness-map-smoke.ts' as const,
