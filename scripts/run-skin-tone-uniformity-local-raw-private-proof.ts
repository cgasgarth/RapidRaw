...
await runRequired('skin-tone committed RAW proof summary', [
  'bun',
  'run',
  'check:skin-tone-uniformity-local-raw-proof',
]);
...
if (args.privateRoot === undefined) {
  ...
  console.log('skin-tone uniformity local RAW private proof skipped (RAWENGINE_PRIVATE_RAW_ROOT unset)');
  process.exit(0);
}
...
if (prepare.stdout.includes('private root skipped')) {
  ...
  console.log('skin-tone uniformity local RAW private proof skipped (local RAW source unavailable)');
  process.exit(0);
}

await runRequired(
  'skin-tone uniformity RAW runtime proof',
  [
    'cargo',
    '+1.95.0',
    'test',
    ...
    'raw_open_edit_export_proof::tests::private_runtime_smoke_generates_skin_tone_uniformity_report_when_enabled',
    '--',
    '--nocapture',
  ],
  {
    ...
    env: {
      ...
      RAWENGINE_RUN_PRIVATE_RAW_SKIN_TONE_UNIFORMITY_PROOF: '1',
    },
  },
);

await runRequired(
  'skin-tone private artifact validation',
  ['bun', 'run', 'check:skin-tone-uniformity-local-raw-proof', '--require-assets'],
  {
    env: {
      RAWENGINE_PRIVATE_RAW_ROOT: privateRoot,
    },
...
console.log('skin-tone uniformity local RAW private proof ok');
