...
import { loadStartupAppAfterShellReceipt } from './product/startupShellHandoff';
...
const shellReceipt = frontendStartupReporter.start().catch((error: unknown) => {
  console.error('Failed to report interactive startup shell:', error);
});

void loadStartupAppAfterShellReceipt(shellReceipt, () =>
  import('./App.js').then(({ AppWrapper }) => ({ AppWrapper })),
).then((result) => {
