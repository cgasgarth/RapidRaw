...
export const AI_APP_SERVER_TOOL_ROUTE_MANIFEST = aiAppServerToolRouteManifestSchema.parse({
  routes: [
    ...
    },
    {
      deferredIssue: '#1276',
      reason:
        'Foreground mask generation needs a dedicated AI mask capability and command payload before app-server apply.',
      status: 'deferred',
      tauriInvoke: 'generate_ai_foreground_mask',
    },
    {
      deferredIssue: '#1276',
      reason: 'Sky mask generation needs a dedicated AI mask capability and command payload before app-server apply.',
      status: 'deferred',
